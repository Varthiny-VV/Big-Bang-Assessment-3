using Microsoft.EntityFrameworkCore;
using TourPackages.Interfaces;
using TourPackages.Models;


namespace TourPackages.Services
{
    public class ItineraryRepo : IRepo<int, Itinerary>
    {
        private readonly Context _context;
        private readonly ILogger _logger;

        public ItineraryRepo(Context tourPackageContext, ILogger<Itinerary> logger)
        {
            _context = tourPackageContext;
            _logger = logger;
        }
        public async Task<Itinerary?> Add(Itinerary item)
        {

            try
            {
                _context.Itinerary.Add(item);
                await _context.SaveChangesAsync();
                return item;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex.Message);
            }
            return null;
        }

        public async Task<Itinerary?> Delete(int key)
        {
            try
            {
                var doc = await Get(key);
                if (doc != null)
                {
                    _context.Itinerary.Remove(doc);
                    await _context.SaveChangesAsync();
                    return doc;
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex.Message);
            }
            return null;
        }

        public async Task<Itinerary?> Get(int key)
        {
            try
            {
                var doc = await _context.Itinerary.FirstOrDefaultAsync(i => i.Id == key);
                return doc;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex.Message);
            }
            return null;
        }

        public async Task<ICollection<Itinerary>?> GetAll()
        {
            try
            {
                var doc = await _context.Itinerary.ToListAsync();
                if (doc.Count > 0)
                    return doc;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex.Message);
            }
            return null;
        }

        public async Task<Itinerary?> Update(Itinerary item)
        {
            try
            {
                var existingDoctor = await _context.Itinerary.FindAsync(item.Id);
                if (existingDoctor != null)
                {
                   
                    existingDoctor.PackageImages = item.PackageImages;
                    
                    existingDoctor.DestinationDescription = item.DestinationDescription;
                    existingDoctor.DayAndDayTitle = item.DayAndDayTitle;

                    await _context.SaveChangesAsync();

                    return existingDoctor;
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex.Message);
            }
            return null;
        }
    }
}
